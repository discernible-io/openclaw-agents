...
if (mode === "--mediated") {
    ...
    console.log("Mediated JWT acquired, len=" + p2pJwt.length);
} else if (mode === "--auto") {
    loginLabel = "Auto";
    console.log("==> Plugin outbound auth mode=auto (P2P first, mediated fallback)");
    ...
    try {
        ...
        console.log("P2P JWT acquired, len=" + p2pJwt.length);
    } catch (e) {
        console.error("P2P login FAILED:", e instanceof Error ? e.message : e);
        process.exit(1);
    }
}
...
if (result.status >= 200 && result.status < 300) {
    console.log("\n" + loginLabel + " peer test PASS");
} else {
    console.error("\n" + loginLabel + " peer test FAIL");
    process.exit(1);
}
