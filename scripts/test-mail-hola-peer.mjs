...
if (!extDir || !credPath || !peerTokenId || !fromEmail) {
  process.stderr.write(
    "usage: test-mail-hola-peer.mjs --ext-dir <a2a> --creds <near.json> --peer-token-id <id> --from-email <addr> [--from-name <name>] [--api-base <url>] [--poll-seconds N]\n",
  );
  process.exit(2);
}
...
async function assessReply(variant, expectedVerified) {
  ...
  if (!hit) {
    reportSkip(
      ...
      `no message with probeId within ${pollSeconds}s — peer may not auto-reply by email`,
    );
    tally.addSkip();
    return;
  }
